...
public class StepIntoBreakpoint extends RunToCursorBreakpoint {
  private static final Logger LOG = Logger.getInstance("#StepIntoBreakpoint");
  @NotNull
  ...
  protected void createRequestForPreparedClass(DebugProcessImpl debugProcess, ReferenceType classType) {
    try {
      ...
      if (!locations.isEmpty()) {
        final MultiMap<Method, Location> methods = new MultiMap<Method, Location>();
        ...
        if (methodsFound == 1) {
          location = methods.entrySet().iterator().next().getValue().iterator().next();
        }
        else {
          if (myFilter instanceof LambdaMethodFilter) {
            ...
            if (lambdaFilter.getLambdaOrdinal() < methodsFound) {
              final Method[] candidates = methods.keySet().toArray(new Method[methodsFound]);

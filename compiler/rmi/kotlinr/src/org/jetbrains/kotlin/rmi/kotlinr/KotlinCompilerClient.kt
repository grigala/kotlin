...
public object KotlinCompilerClient {
    ...
    public fun compile(compilerService: CompileService, compilerFlavor: CompileService.CompilerImplementation, args: Array<out String>, out: OutputStream): Int {

        val outStrm = RemoteOutputStreamServer(out)
        val servicesFacade = CompilerCallbackServicesFacadeServer()
        try {
            return compilerService.remoteCompile(compilerFlavor, args, servicesFacade, outStrm, CompileService.OutputFormat.PLAIN, outStrm)
        }
        finally {
    ...
    public fun incrementalCompile(compileService: CompileService,
                                  compilerFlavor: CompileService.CompilerImplementation,
        ...
        try {
            return profiler.withMeasure(this) {
                compileService.remoteIncrementalCompile(compilerFlavor, args, servicesFacade, compilerOutStreamServer, CompileService.OutputFormat.XML, daemonOutStreamServer)
            }
        }
        finally {
            servicesFacade.disconnect()
        ...
        if (daemon == null) {
            ...
            filteredArgs.none() -> {
                ...
                try {
                    ...
                    val res = daemon.remoteCompile(CompileService.CompilerImplementation.JVM, filteredArgs.toArrayList().toTypedArray(), servicesFacade, outStrm, CompileService.OutputFormat.PLAIN, outStrm)
